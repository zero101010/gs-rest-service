[![Service Availability Monitor](https://github.com/zero101010/gs-rest-service/actions/workflows/monitor.yml/badge.svg)](https://github.com/zero101010/gs-rest-service/actions/workflows/monitor.yml)
...
To do so, provide a Java record class (in `src/main/java/com/example/restservice/Greeting.java`) or a Kotlin data class (in ``src/main/kotlin/com/example/restservice/Greeting.kt`) for the `id` and `content` data, as the following listing shows:
...
Create a resource controller (in `src/main/java/com/example/restservice/GreetingController.java` for Java or `src/main/kotlin/com/example/restservice/GreetingController.kt` for Kotlin) with the following contents:
...
https://github.com/FasterXML/jackson[Jackson] is on the classpath, Spring's
https://docs.spring.io/spring/docs/{spring_version}/javadoc-api/org/springframework/http/converter/json/JacksonJsonHttpMessageConverter.html[`JacksonJsonHttpMessageConverter`]
...
to further modify the class. The following listing shows the `RestServiceApplication` application class (in `src/main/java/com/example/restservice/RestServiceApplication.java`
 for Java or `src/main/kotlin/com/example/restservice/RestServiceApplication.kt`
for Kotlin):
...
* https://spring.io/guides/gs/cloud-circuit-breaker/[Circuit Breaker]
